...
class ReservasiController extends Controller
{
    public function index(Request $request)
    {
        ...
        $reservasi  = Reservasi::orderBy('t_reservasi.tanggal_reservasi', 'DESC')->whereYear('tanggal_reservasi', 2026);
        $bulan      = [];
        $bulanPick  = [];
        $statusPick = [];
    ...
    public function select(Request $request)
    {
        ...
        foreach ($result as $row) {
            ...
            $response[] = [
                ...
                'aksi'      => $aksi,
                'detail'    => $row->detail,
                'masuk'     => Carbon::parse($row->tanggal_masuk)->isoFormat('dddd, DD MMMM Y'),
                'keluar'    => Carbon::parse($row->tanggal_keluar)->isoFormat('dddd, DD MMMM Y'),
